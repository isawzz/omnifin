...
async function filterRecordsWORKS(allowEdit = false) {
	...
	if (compExp) {
		...
		havingItems = selitems.filter(x => tagNames.includes(x.h)); console.log('havingItems', havingItems);
		compExp = sqlBuildCompareExp(havingItems, clauses); console.log('compExp',compExp)		
		if (isdef(clauses.HAVING)) {
			let cl = clauses.HAVING[0];
			clauses.HAVING = [`HAVING ` + stringAfter(cl, 'HAVING') + ' AND ' + compExp];
		}
...
function clsGetHeaderMapping1_vorCorrection(clauses, sorting) {
