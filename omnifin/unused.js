...
function sqlBuildCompareExp(items, clauses, key='WHERE') {
	...
	if (isdef(clauses[key])) {
		...
		for (const p of parts) {
			...
			let item = items.find(x => x.header == header && x.text == text);
			if (isdef(item)) removeInPlace(items, item);
		}
	}
...
function sqlUpdateOrderBy(q, sorting) {
	let clauses = sqlSplitClauses(q); // console.log('clauses',clauses)
	let qnew = '';
